...
namespace WarAndAiTweaks.Patches {
    ...
    public class SpawnLordPartyInternalPatch {
        ...
        public static void Prefix(PartyTemplateObject pt, ref int troopNumberLimit, ref MobileParty __instance) {
            ...
            if (!WarAndAiTweaks.Settings.EnableFillStacks)
                return;

            if (__instance.LeaderHero != null && __instance.LeaderHero.Clan != null)
                return;

            if (!__instance.IsLordParty)
                return;

            if (__instance.LeaderHero.Clan.IsUnderMercenaryService && !WarAndAiTweaks.Settings.FillStackMercenaries)
                return;
